...
const source = {
  ...
  proposal: read('src/components/ProposalDialog.tsx'), review: read('src/components/ReviewDialog.tsx'), constraints: read('src/components/CalendarConstraintManager.tsx'),
...
add('版本与迁移', '发布版本为 0.8.0', pkg.version === '0.8.0', `package.json: ${pkg.version}`)
add('版本与迁移', '状态架构版本提升为 8', /SCHEMA_VERSION\s*=\s*8/.test(source.types), 'src/types.ts')
...
add('目标系统', '最近相关期限，不按目标数量加权', /nearestRelevantGoalDate/.test(source.goals) && /sort\(\)/.test(source.goals), 'src/lib/goals.ts')
...
add('可解释方案', '任务、日期、负载、目标均有前后对比', /before-after/.test(source.proposal) && /日期负载前后/.test(source.proposal) && /目标影响/.test(source.proposal), 'ProposalDialog')
add('可解释方案', '解释为什么不选其他日期', /为什么没有安排到其他日期/.test(source.proposal) && /rejectedAlternatives/.test(source.planner), 'proposal + planner')
add('可解释方案', '更多替代方案按结果签名去重', /distinctSignature/.test(source.planner) && /signatures\.has/.test(source.planner) && /真正不同/.test(source.proposal), 'planner + UI')
add('复盘与时长', '复盘默认摘要优先，图表按需展开', /review-summary-grid/.test(source.review) && /展开统计图表/.test(source.review), 'ReviewDialog')
...
add('复盘与时长', '时长建议不静默覆盖，接受后仍进入方案', /prepareDurationChange/.test(source.context) && /不覆盖完成历史|不会直接覆盖/.test(source.review), 'Review + AppContext')
