...
const result = { version: '0.8.10', generatedAt: new Date().toISOString(), passed, total: checks.length, checks }
fs.mkdirSync(path.join(root, 'validation'), { recursive: true })
fs.writeFileSync(path.join(root, 'validation', 'v0.8.10用户效率验证.json'), JSON.stringify(result, null, 2))
const lines = ['# Study Planner v0.8.10 用户效率验证', '', `- 通过：${passed} / ${checks.length}`, `- 生成时间：${result.generatedAt}`, '', ...checks.map(item => `- ${item.pass ? '✅' : '❌'} **${item.name}**：${item.evidence}`)]
fs.writeFileSync(path.join(root, 'validation', 'v0.8.10用户效率验证.md'), lines.join('\n') + '\n')
