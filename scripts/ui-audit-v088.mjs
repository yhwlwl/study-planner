...
const output = { version: '0.8.8', generatedAt: new Date().toISOString(), passed, total: checks.length, checks }
fs.mkdirSync(path.join(root, 'validation'), { recursive: true })
fs.writeFileSync(path.join(root, 'validation', 'v0.8.8界面布局审计.json'), JSON.stringify(output, null, 2))
const md = ['# Study Planner v0.8.8 界面布局审计', '', `- 通过：${passed} / ${checks.length}`, `- 生成时间：${output.generatedAt}`, '', ...checks.map(item => `- ${item.pass ? '✅' : '❌'} **${item.name}**：${item.evidence}`)]
fs.writeFileSync(path.join(root, 'validation', 'v0.8.8界面布局审计.md'), md.join('\n') + '\n')
console.log(md.join('\n'))
