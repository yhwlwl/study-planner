...
add('计划调整中心按动作分组并支持自适应网格', adjustment.includes('adjustment-action-groups') && adjustment.includes('adjustment-action-grid') && css.includes('.adjustment-action-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))') && css.includes('.adjustment-action-grid,.adjustment-action-group:nth-child(2) .adjustment-action-grid{grid-template-columns:1fr'), '桌面按场景分组、平板两列、手机单列')
add('计划调整表单手机回落单栏', adjustment.includes('adjustment-form-grid') && css.includes('.adjustment-form-grid,.direct-operation-form{grid-template-columns:1fr}'), '日期、容量和重排条件在手机端单列')
...
const output = { version: releaseVersion, generatedAt: new Date().toISOString(), passed, total: checks.length, checks }
fs.mkdirSync(path.join(root, 'validation'), { recursive: true })
fs.writeFileSync(path.join(root, 'validation', `v${releaseVersion}界面布局审计.json`), JSON.stringify(output, null, 2))
const md = [`# Study Planner v${releaseVersion} 界面布局审计`, '', `- 通过：${passed} / ${checks.length}`, `- 生成时间：${output.generatedAt}`, '', ...checks.map(item => `- ${item.pass ? '✅' : '❌'} **${item.name}**：${item.evidence}`)]
fs.writeFileSync(path.join(root, 'validation', `v${releaseVersion}界面布局审计.md`), md.join('\n') + '\n')
console.log(md.join('\n'))
