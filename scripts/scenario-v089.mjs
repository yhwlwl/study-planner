...
add('单任务删除不借机移动其他任务',
  /prepareAssignmentDelete/.test(context) && /type:\s*'assignment-deletion'/.test(context)
    && /仅移除/.test(context) && /不会移动其他任务/.test(app)
...
fs.writeFileSync(path.join(root, 'validation', 'v0.8.9场景架构验证.json'), JSON.stringify(output, null, 2))
const md = ['# Study Planner v0.8.9 场景架构验证', '', `- 通过：${passed} / ${results.length}`, `- 生成时间：${output.generatedAt}`, '', ...results.map(item => `- ${item.pass ? '✅' : '❌'} **${item.scenario}**：${item.evidence}`)]
fs.writeFileSync(path.join(root, 'validation', 'v0.8.9场景架构验证.md'), md.join('\n') + '\n')
console.log(md.join('\n'))
