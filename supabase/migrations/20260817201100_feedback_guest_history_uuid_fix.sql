-- feedback_submissions.visitor_id 是 uuid；修正游客 RPC 参数类型并移除旧 text 签名。

drop function if exists public.submit_guest_feedback(text, text, text, text);
drop function if exists public.list_guest_feedback(text, text);
...
revoke all on function public.submit_guest_feedback(text, text, uuid, text) from public, authenticated;
grant execute on function public.submit_guest_feedback(text, text, uuid, text) to anon;
...
comment on function public.submit_guest_feedback(text, text, uuid, text) is
  'Creates guest feedback using a per-browser secret; direct anonymous table inserts are intentionally disabled.';
comment on function public.list_guest_feedback(uuid, text) is
