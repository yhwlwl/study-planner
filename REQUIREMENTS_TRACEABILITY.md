...
## 十一、v0.8.6 冲突决策与移动端修复追踪

- [x] 手机 Today 头部不再受桌面 flex-basis 高度影响。
- [x] 硬冲突按绝对阻断、用户保护、可一次性例外、目标/结构和提醒分类。
- [x] 每个冲突可展开到涉及任务、当前值、允许值、后果和解决方式。
- [x] 用户可分别接受或拒绝多个一次性例外。
- [x] 已接受例外作为新计算条件，未接受例外不被自动重新加入。
- [x] 重新计算从当前选中候选继续，保留其余合法变化。
- [x] 一次性例外精确到日期、规则、临时上限和任务。
- [x] 冲突状态不再通过简单禁用按钮形成死路。
- [x] 无候选时提供扩大搜索、修改容量和修改目标入口。
- [x] 所有重算结果再次完整预览后才能应用。

## 十二、v0.8.7 复盘与已完成历史边界追踪

- [x] 复盘完成页提供按当前方案顺延、获取更多方案、仅保存复盘三条路径。
- [x] 当前复盘方案合法时只校验并直接提交，不运行完整多方案搜索。
- [x] 只有用户主动获取更多方案时才进入完整计划调整中心。
- [x] 已完成任务与实际用时不生成待处理硬冲突。
- [x] 已完成历史仍计入当天真实负载和各类上限。
- [x] 新的未完成任务进入历史超载日期时仍接受完整校验。
- [x] 只把新增或仍可调整任务列为冲突涉及任务。
- [x] 原有非法状态得到改善但尚未消失时不被误判为新冲突。
- [x] 冲突操作按处理任务、修改条件、撤销变化分层。
- [x] 部分接受与拒绝例外后仍重新计算并再次预览。
- [x] 复盘合法任务在冲突处理和更多方案中保持明确基线。
- [x] 新增运行级测试覆盖 9 小时、5 套卷子和历史超载日新增任务。

## 十三、v0.8.8 桌面与手机界面分层追踪

| 新增/修正要求 | 实现位置 | 验证 |
|---|---|---|
| 复盘电脑端不再使用六张过窄指标卡 | `ReviewDialog.tsx`、`styles.css` 的 `review-modal` 与三列摘要 | `npm run test:ui` |
| 计划调整电脑端主次信息分区 | `AdjustmentIntentDialog.tsx` 的 `adjustment-layout` | UI 审计 15/15 |
| 今日可接收容量不留大面积空白 | `adjustment-today-control` 2×2 卡片 | UI 审计 |
| 方案预览桌面与手机分别排版 | `proposal-modal`、桌面四列/手机两列摘要 | UI 审计 |
| 复杂弹窗桌面独立滚动、手机 100dvh | `Modal.tsx` 页面级 class + `styles.css` | UI 审计 |
| 其他目标/统计/设置/任务/约束界面保持一致 | 全局卡片、表单与列表覆盖规则 | UI 审计 |
| 不因美化删除详细解释 | Proposal/Review 详情结构未删减 | 36 场景 + 72 综合验证 |


## v0.8.10 用户效率与可信度专项

- [x] 正在计时的任务完成前必须结算计时，极端陈旧状态仍记录已计时分钟。
- [x] Today 区分原计划、实际执行、剩余预计和执行负载，并与复盘口径可解释对应。
- [x] 每日统计抽取为可独立测试的纯函数，旧数据残差只记账一次。
- [x] “任务”页面提供待处理、未安排、逾期、今日、未来、已完成和全部视图。
- [x] 复盘仅保存但暂不顺延的任务仍可从任务收件箱和首页提醒找到。
- [x] 批量顺延、删除、导入和恢复均只保留一个最终确认点。
- [x] 任务标题、预计时长和日期备注使用草稿一次保存，不按字符消耗撤销历史。
- [x] 时长变化先执行用户明确修改，只有新增或恶化问题才进入修复。
- [x] 批量移动使用日期选择器，不再使用浏览器文本输入。
- [x] 导入、云端恢复和冲突备份恢复先展示覆盖差异，并支持就近撤销。
- [x] 算法参数、维护恢复、任务组高级规则和特殊目标关联默认折叠。
- [x] 长表单不会因误点背景关闭。
- [x] 编号整理作为非阻断结果条出现，用户主动查看后再决定。
- [x] 手机端关键图标操作提供可见文字标签。
