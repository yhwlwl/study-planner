...
export function IntakePage({ onPrepared, onNavigate, onAddTask, addRequest, onAddRequestHandled, tutorialMode = false, onStartTutorial }: {
  ...
  const {
    ...
    removeIntakeTaskGroup, deleteIntakeBatch, prepareIntakeBatch,
  } = useApp()
  ...
  return <div className="intake-page">
    <section className="intake-intro">
      ...
      {!tutorialMode && <button className="primary-button" onClick={createBatch}><FolderPlus size={17}/>新建录入批次</button>}
    </section>
    ...
    <div className="intake-layout">
      <aside className="intake-batch-list" aria-label="录入批次">
        <div className="intake-batch-list-head"><strong>录入批次</strong>{!tutorialMode && <button className="text-button" onClick={() => setShowArchived(value => !value)}>{showArchived ? '隐藏归档' : `归档 ${state.intakeBatches.length - activeBatches.length}`}</button>}</div>
      ...
      <section className="intake-workspace">
        {!active ? <div className="intake-empty-main">
          ...
          {!tutorialMode && <div className="intake-empty-options">
            ...
            {!state.assignments.length && onStartTutorial && <button className="secondary-button" onClick={onStartTutorial}><FileSpreadsheet size={17}/><span><strong>体验完整流程</strong><small>在独立教程空间里亲手走一遍</small></span></button>}
          </div>}
        </div> : <>
          <header className="intake-workspace-head">
            ...
            <div className="intake-head-actions">{!tutorialMode && <>
          ...
          <div className="intake-next-step">
            ...
            <div>{!tutorialMode && <>{!state.assignments.length && <><button className="secondary-button" onClick={() => onNavigate('goals')}>设置目标</button><button className="secondary-button" onClick={() => onNavigate('settings')}>设置可用时间</button></>}<button className="text-button" onClick={() => updateIntakeBatch(active.id, { status: 'archived' })}><Archive size={15}/>归档批次</button></>}</div>
          </div>
        </>}
      </section>
    </div>
