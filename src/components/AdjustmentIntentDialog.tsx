  ...
  const renderReplan = () => {
    ...
    return <>
      ...
      <section className="adjustment-form-section">
        ...
        <fieldset className="adjustment-choice-fieldset"><legend>这次重排采用什么取舍？</legend><div className="adjustment-preference-options">{(['preserve', 'balanced', 'goal', 'rest'] as ReplanOutcome[]).map(item => { const selected = replanOutcome === item; return <button type="button" key={item} aria-pressed={selected} className={selected ? 'selected' : ''} disabled={tutorialMode === 'future' && item !== 'goal'} onClick={() => setReplanOutcome(item)}><span className="choice-indicator">{selected ? '已选择' : '可选'}</span><strong>{preferenceCopy[item].title}</strong><span>{preferenceCopy[item].description}</span></button> })}</div></fieldset>
