    ...
    <main className={`focus-timer-page ${timer.running ? 'is-running' : 'is-paused'}`}>
      ...
      <Modal open={finishOpen} title="结束本次计时" onClose={closeFinish}>
        ...
        <div className="form-stack">
          <label className="field"><span>记入实际用时（分钟）</span><input type="number" min="1" step="1" value={sessionMinutes} onChange={event => setSessionMinutes(event.target.value)} autoFocus/></label>
          <label className="field"><span>保存为部分完成时的当前进度</span><input type="number" min="1" max="99" value={progress} onChange={event => setProgress(Number(event.target.value))}/></label>
        </div>
