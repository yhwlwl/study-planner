...
import { Eye, Link2, Plus, RotateCcw, Settings, Target, Trash2 } from 'lucide-react'
import type { AppState, Goal, GoalCondition, GoalConditionMode, GoalDraft, PlanChangeEvent, Priority } from '../types'
import { useApp } from '../AppContext'
import { goalProgress } from '../lib/goals'
import { uid } from '../lib/id'
import { fmtDate, minutesText, shiftDate } from '../lib/date'
...
export function GoalsPage({ onPrepared, tutorialMode = false, tutorialStep, tutorialAnchorDate, onTutorialExistingViewed, onTutorialGoalCreated, onTutorialGoalLinked, onTutorialBlocked }: { onPrepared: (prepared: AppState, event: PlanChangeEvent) => void; tutorialMode?: boolean; tutorialStep?: TutorialStep; tutorialAnchorDate?: string; onTutorialExistingViewed?: () => void; onTutorialGoalCreated?: () => void; onTutorialGoalLinked?: () => void; onTutorialBlocked?: (message?: string) => void }) {
  ...
  const tutorialPendingItems = useMemo(() => state.intakeBatches.find(batch => batch.id === TUTORIAL_INTAKE_BATCH_ID)?.taskGroups.filter(item => !item.appliedAt) ?? [], [state.intakeBatches])
  const tutorialNewGoal = state.goals.find(goal => goal.id === TUTORIAL_NEW_GOAL_ID || goal.title === TUTORIAL_NEW_GOAL_TITLE)

  const save = (draft: GoalDraft, goalId?: string) => {
    if (tutorialMode && tutorialStep === 'goal-create' && !goalId) {
      const now = new Date().toISOString()
      commit(next => {
        if (next.goals.some(goal => goal.id === TUTORIAL_NEW_GOAL_ID)) return
        next.goals.push({ id: TUTORIAL_NEW_GOAL_ID, title: draft.title, description: draft.description, priority: draft.priority, desiredDate: draft.desiredDate, latestDate: draft.latestDate, status: 'active', completionConditions: [], linkedTaskGroupIds: [], linkedAssignmentIds: [], createdAt: now, updatedAt: now })
      }, { history: false, tutorialAction: 'goal-create' })
  ...
  return <div className="goals-page">
    <div className="section-toolbar"><div><h2>目标</h2><p>目标定义要完成什么以及期望与最晚日期；修改目标只生成建议，不直接移动任务。</p></div><button data-tutorial-target={tutorialStep === 'goal-create' ? 'tutorial-goal-create' : undefined} className={`primary-button ${tutorialMode && tutorialStep !== 'goal-create' ? 'tutorial-disabled-control' : ''}`} aria-disabled={tutorialMode && tutorialStep !== 'goal-create' || undefined} onClick={() => tutorialMode && tutorialStep !== 'goal-create' ? onTutorialBlocked?.() : setEditing(null)}><Plus size={17}/>创建目标</button></div>
    ...
    <GoalDialog open={editing !== undefined} state={state} initial={editing ?? undefined} tutorialDefaults={tutorialMode && tutorialStep === 'goal-create' && tutorialAnchorDate ? { title: TUTORIAL_NEW_GOAL_TITLE, description: '教程：给刚录入的新作业设置共同期限', priority: 3, desiredDate: shiftDate(tutorialAnchorDate, 5), latestDate: shiftDate(tutorialAnchorDate, 7), completionConditions: [], linkedTaskGroupIds: [], linkedAssignmentIds: [] } : undefined} readOnly={tutorialMode && tutorialStep === 'goal-create'} onClose={() => setEditing(undefined)} onSave={save}/>
    <Modal open={Boolean(detailGoal)} title={detailGoal?.title ?? '目标详情'} onClose={() => setDetailGoal(undefined)} wide>
      ...
      <div className="modal-actions"><button className="secondary-button" onClick={() => setDetailGoal(undefined)}>关闭</button>{tutorialStep === 'goal-existing' && detailGoal?.id === TUTORIAL_GOAL_ID && <button className="primary-button" data-tutorial-target="tutorial-goal-detail-continue" onClick={() => { setDetailGoal(undefined); onTutorialExistingViewed?.() }}>继续</button>}</div>
    </Modal>
    <Modal open={linkOpen} title="关联新录入任务" onClose={() => setLinkOpen(false)} wide>
      <p>把刚才识别的任务关联到“{tutorialNewGoal?.title ?? TUTORIAL_NEW_GOAL_TITLE}”。这些任务正式生成后，会继承这个目标的期限约束。</p>
      <div className="tutorial-pending-list">{tutorialPendingItems.map(item => <label className="tutorial-pending-item" key={item.id}><span><input type="checkbox" checked={linkIds.includes(item.id)} onChange={event => setLinkIds(current => event.target.checked ? [...new Set([...current, item.id])] : current.filter(id => id !== item.id))}/> {item.title}</span><em>{item.quantity} 项 · {item.unitMinutes} 分钟/项</em></label>)}</div>
      <div className="modal-actions"><button className="secondary-button" onClick={() => setLinkOpen(false)}>取消</button><button data-tutorial-target="tutorial-goal-link-confirm" className="primary-button" disabled={!tutorialPendingItems.length || linkIds.length !== tutorialPendingItems.length} onClick={() => { if (!tutorialNewGoal) return; commit(next => { const batch = next.intakeBatches.find(item => item.id === TUTORIAL_INTAKE_BATCH_ID); if (!batch) return; for (const item of batch.taskGroups) if (linkIds.includes(item.id)) item.goalIds = [...new Set([...item.goalIds, tutorialNewGoal.id])]; batch.updatedAt = new Date().toISOString() }, { history: false, tutorialAction: 'goal-link' }); setLinkOpen(false); onTutorialGoalLinked?.() }}>确认关联</button></div>
    </Modal>
  </div>
}

function GoalDialog({ open, state, initial, tutorialDefaults, readOnly = false, onClose, onSave }: { open: boolean; state: AppState; initial?: Goal; tutorialDefaults?: GoalDraft; readOnly?: boolean; onClose: () => void; onSave: (draft: GoalDraft, goalId?: string) => void }) {
  const [title, setTitle] = useState(initial?.title ?? tutorialDefaults?.title ?? '')
  const [description, setDescription] = useState(initial?.description ?? tutorialDefaults?.description ?? '')
  const [priority, setPriority] = useState<Priority>(initial?.priority ?? tutorialDefaults?.priority ?? 3)
  const [desiredDate, setDesiredDate] = useState(initial?.desiredDate ?? tutorialDefaults?.desiredDate ?? '')
  const [latestDate, setLatestDate] = useState(initial?.latestDate ?? tutorialDefaults?.latestDate ?? state.settings.endDate)
  const [conditions, setConditions] = useState<GoalCondition[]>(initial?.completionConditions ?? tutorialDefaults?.completionConditions ?? [])
  const [assignmentIds, setAssignmentIds] = useState<string[]>(initial?.linkedAssignmentIds ?? tutorialDefaults?.linkedAssignmentIds ?? [])
  const [assignmentQuery, setAssignmentQuery] = useState('')
  const resetKey = `${open}-${initial?.id ?? 'new'}`
  useEffect(() => { setTitle(initial?.title ?? tutorialDefaults?.title ?? ''); setDescription(initial?.description ?? tutorialDefaults?.description ?? ''); setPriority(initial?.priority ?? tutorialDefaults?.priority ?? 3); setDesiredDate(initial?.desiredDate ?? tutorialDefaults?.desiredDate ?? ''); setLatestDate(initial?.latestDate ?? tutorialDefaults?.latestDate ?? state.settings.endDate); setConditions(initial?.completionConditions ?? tutorialDefaults?.completionConditions ?? []); setAssignmentIds(initial?.linkedAssignmentIds ?? tutorialDefaults?.linkedAssignmentIds ?? []); setAssignmentQuery('') }, [resetKey, initial, state.settings.endDate, tutorialDefaults])
  ...
  return <Modal open={open} title={initial ? '编辑目标' : '创建目标'} onClose={onClose} wide mobileFullscreen>
    ...
    </div></fieldset><div className="modal-actions"><button className="secondary-button" onClick={onClose}>取消</button><button data-tutorial-target={readOnly ? "tutorial-goal-create-confirm" : undefined} className="primary-button" disabled={!title.trim() || !latestDate || Boolean(desiredDate && desiredDate > latestDate)} onClick={submit}>{!initial && conditions.length === 0 && assignmentIds.length === 0 ? '保存目标草稿' : '保存并预览影响'}</button></div>
  </Modal>
}
