...
export function GoalsPage({ onPrepared, tutorialMode = false }: { onPrepared: (prepared: AppState, event: PlanChangeEvent) => void; tutorialMode?: boolean }) {
  ...
  return <div className="goals-page">
    <div className="section-toolbar"><div><h2>目标</h2><p>目标定义要完成什么以及期望与最晚日期；修改目标只生成建议，不直接移动任务。</p></div>{!tutorialMode && <button className="primary-button" onClick={() => setEditing(null)}><Plus size={17}/>创建目标</button>}</div>
    {goals.length === 0 ? <div className="empty-state"><h3>还没有目标</h3><p>{tutorialMode ? '教程数据正在恢复，请稍候。' : '创建目标后，可以为一个或多个任务组设置全部、百分比或数量完成条件。'}</p>{!tutorialMode && <button className="primary-button" onClick={() => setEditing(null)}>创建第一个目标</button>}</div> : <div className="goal-grid">{goals.map(goal => {
      ...
      return <article className={`goal-card goal-${goal.status}`} data-tutorial-target={goal.id === 'tutorial-goal-math' ? 'tutorial-goal' : undefined} key={goal.id}>
        <header><div><div className="goal-card-kicker"><span className="status-pill">{goal.status === 'active' ? '进行中' : goal.status === 'completed' ? '已完成' : '已归档'}</span><span className="goal-priority-pill">{goal.priority === 5 ? '核心' : goal.priority === 3 ? '高' : goal.priority === 2 ? '中' : goal.priority === 1 ? '低' : '可选'}</span></div><h3>{goal.title}</h3>{goal.description && <p>{goal.description}</p>}</div><div className="row-actions">{!tutorialMode && <><button className="icon-button" title="编辑" onClick={() => setEditing(goal)}><Settings size={17}/><span className="mobile-action-label">编辑</span></button><button className="icon-button" disabled={goal.status === 'active'} title={goal.status === 'active' ? '目标完成后才能归档' : goal.status === 'archived' ? '取消归档' : '归档'} onClick={() => toggleArchive(goal)}>{goal.status === 'archived' ? <RotateCcw size={17}/> : <Target size={17}/>}<span className="mobile-action-label">{goal.status === 'archived' ? '取消归档' : '归档'}</span></button><button className="icon-button danger" aria-label={`删除目标${goal.title}`} onClick={() => { const prepared = prepareGoalDelete(goal.id); onPrepared(prepared.state, prepared.event) }}><Trash2 size={17}/><span className="mobile-action-label">删除</span></button></>}</div></header>
